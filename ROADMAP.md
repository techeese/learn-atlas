...
2. **Deepen the existing six** — ✅✅ COMPLETE (iter 99). Every original topic now has a 7th frontier/advanced module,
   ...
   ✅ iter 647: **NEW viz algo-trie** (111th). ✅ iter 648: **NEW viz algo-maxflow** (112th — live Edmonds-Karp + min-cut, cut shifts source→sink; algos 21/23→22/23). gate GREEN, headless duality+cut-shift, screenshot, SW →v588.
   ⏳ NEW-viz queue (remaining are conceptual/discussion lessons where a forced viz risks gilding): a-np-completeness; calc c-integration-techniques, c-intro-differential-equations(done); RL tail (value-approximation/model-based/offline/imitation); LLM tail (optimization-stability/hallucination-eval/safety); DL transfer-learning. Build ONLY where genuinely visual & high-value; otherwise the viz frontier is essentially complete (112 widgets, ~93% lesson coverage).
