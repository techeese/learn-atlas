...
**11 topics · 193 lessons · 177 visualizations · 595 glossary terms · 168 code exercises** (Game Theory fully built out: 3 lessons + deep-dives on mechanism design/auctions, evolutionary GT/ESS, cooperative games/Shapley, and Bayesian games/signaling — integrated with viz, glossary, code, refs, prereqs)
...
- **data-viz embeds come in both quote styles (iter 1148/1149):** older lessons use `data-viz='id'` (single quotes) — any scan/lint over embeds must be quote-agnostic or it reports false orphans. gate.js now validates both styles AND fails on duplicate widget registrations (a dup silently shadows the earlier widget and double-lists it in the Lab). Also: when writing negative tests via `node -e` in shell single quotes, inner single quotes get stripped — use \x27 escapes or the test tests nothing.
