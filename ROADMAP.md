...
2. **Deepen the existing six** — ✅✅ COMPLETE (iter 99). Every original topic now has a 7th frontier/advanced module,
   ...
   ⏳ NEW-viz build queue (no existing fit): algos a-network-flow (max-flow/min-cut), a-np-completeness, a-string-algorithms (KMP/trie); calc c-integration-techniques; RL tail (value-approximation, model-based, offline, imitation); LLM tail (optimization-stability, peft-lora, hallucination-eval, safety). Build one per iter where high-value & visual.
