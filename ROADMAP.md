...
2. **Deepen the existing six** — ✅✅ COMPLETE (iter 99). Every original topic now has a 7th frontier/advanced module,
   ...
   ✅ iter 607: **Flashcard parity for loop-built topics** (content/SRS). ML+IT had 5 cards/lesson vs 6 elsewhere; added a complementary 6th card to all 17 ML+IT
   lessons (deduped, render-guarded). Now all at 6. Verified: gate GREEN (render-lint on 17 new cards), 0/17 off-count, headless Recall deck kErr=0/errs=0. SW →v548.
   ✅ iter 608: **MCQ top-up batch 1 — it-entropy 8→16** (content/assessment). 8 new distinct MCQs, all numbers node-verified, balanced (combined 4/4/4/4),
   ...
   ✅ iter 648: **NEW viz algo-maxflow** (112th). ✅ iter 649: **NEW viz algo-pnp** (113th — P/NP/NP-complete map; **Algorithms now 23/23 viz-complete**). gate GREEN, headless toggle, screenshot, SW →v589.
   ⏳ NEW-viz queue — only genuinely-conceptual lessons remain (a forced viz would gild): calc c-integration-techniques; RL value-approximation/model-based/offline/imitation; LLM optimization-stability/hallucination-eval/safety; DL transfer-learning. The viz frontier is essentially COMPLETE: 113 widgets, ~95% lesson coverage, every topic strong.
   ✅ iter 650: **step-back sweep (165 lessons, errs=0) + "Visualization of the day" dashboard card** (surfaces the 113-viz catalog; deterministic daily pick → #/lab). gate GREEN, headless navigates, screenshot clean, SW →v590.
   ...
   ✅ iter 667: **Bayesian lessons → prereq graph**. ✅ iter 668: **deep-dives parity fix — ps-bayesian-inference +3** (priors spectrum, likelihood principle, log-space/MAP). PARITY GAP: the 4 Bayesian lessons were built with 0 deep-dives (rest of site = 3/lesson).
   ✅ iters 668–671: deep-dives added to all 4 Bayesian lessons (3 each). **MODULE 6 NOW AT FULL SITE PARITY on every dimension: content · 16 MCQ · 3 ex · 3 hw · 6 cards · viz · code · prereqs · 3 deep-dives.**
   ✅ iter 673: NEW viz it-differential-entropy-viz (115th; IT 6/7). ✅ iter 674: **NEW viz rl-value-approx** (116th — tile-coding value approximation; RL 14/20→15/20). gate GREEN, headless error 0.169→0.021, screenshot, SW →v612.
   ✅ iter 675: Library +3 Bayesian references. ✅ iter 676: cross-embedded dl-cross-entropy + dl-kl-divergence into it-information-in-ml → **IT viz 7/7 COMPLETE**.
   ✅ iter 677: **NEW viz dl-transfer** (117th — freeze vs fine-tune; DL 21/22→22/22 COMPLETE).
   ✅ iter 678: NEW viz llm-calibration (118th). ✅ iter 679: **NEW lesson ps-causation-confounding** (correlation≠causation, Simpson's paradox, confounding, RCTs, do-operator; viz+code+3dd+8 MCQ). 170 lessons; PS Module 3 now 4 lessons.
   ✅ iter 680: topped ps-causation-confounding 8→16 MCQ (colliders, IV, do-operator, selection bias, …). Now at full parity; gate parity-warning cleared. 170 lessons all at standard.
   Possible follow-on causal lessons if owner wants depth: backdoor/propensity scores, instrumental variables, causal ML / DAGs.
   ✅ iter 681: cross-embedded dl-optimizers + dl-lr-schedules into l-optimization-and-stability (LLM 17/19→18/19). LLM has 1 no-viz left (l-safety-and-frontier — pure discussion, skip).
   ✅ iter 682: cross-embedded rl-gridworld into rl-model-based (planning §4) → RL 15/20→16/20.
   ✅ iter 683: **NEW viz causal-dag** (119th — fork/chain/collider + d-separation; embedded in ps-causation-confounding). Causal thread now has 1 lesson + its keystone viz.
   ✅ iter 684: **NEW lesson ps-causal-graphs** (DAGs, backdoor criterion, d-separation, adjustment formula; causal-dag viz + code + 3dd + 8 MCQ). 171 lessons; PS Module 3 now 5 lessons. Causal thread = 2 lessons.
   ✅ iter 685: topped ps-causal-graphs 8→16 MCQ. ✅ iter 686: moved both causal lessons into a dedicated **PS Module 7 "Causal Inference"** (M3 back to 3 dependence lessons). PS now 7 modules.
   ✅ iter 687: **NEW lesson ps-causal-estimation** (RCT/A-B, propensity, IV/Wald, DiD; ps-ci-coverage viz + DiD code + 3dd + 8 MCQ). 172 lessons; Causal Inference module now 3 lessons.
   ✅ iter 688: topped ps-causal-estimation 8→16 MCQ → **Causal Inference module COMPLETE at full parity** (3 lessons: confounding → graphs/backdoor → estimation). All 172 lessons at standard.
   ✅ iter 689: step-back full sweep (172 lessons, errs=0/bad=none) + **12 causal glossary terms** (glossary 231→243).
   ✅ iter 690: audit (achievements dynamic, copy accurate, mobile passes — no bug) + **causal-dag active-state buttons** (.viz-btn.active + aria-pressed). NOTE: .viz-btn.active is now a reusable selected-state for any toggle/radio viz buttons.
   ✅ iter 691: audit (defer/daily-picks/gamification all healthy) + **hard-concept deep-dive: why attention divides by √dₖ** (dl-attention-mechanism → 4 deep-dives). Owner direction: hard-concept extra explanations.
   ...
   ✅ iter 696: prereq edge **ml-svm → c-duality-kkt** (SVM solved via duality) — wires the optimization backbone into the Knowledge Map.
   ✅ iter 697: **+10 optimization/duality glossary terms** (Lagrangian, dual function, weak/strong duality, KKT, complementary slackness, Slater, shadow price, convex set). Glossary 243→253. Duality arc now fully integrated.
   ...
   ⏳ Top c-proximal-projected 8→16 MCQ. ★ Optimization arc now spans convexity → GD-convergence → Lagrange → duality/KKT → projected/proximal — a complete backbone. Vary THEME next (not more optimization).
   ...
   ⏳ Remaining no-viz lessons are mostly conceptual/discussion (RL practical/model-based/offline/imitation/frontiers, LLM optimization/eval/safety, DL transfer-learning, calc integration-techniques, la-matrix-derivative-identities, it-information-in-ml) — build a NEW viz only where genuinely visual.
   ★ The Bayesian arc (iters 652–671) is 100% COMPLETE & exhaustively integrated. PS = 6 modules / 24 lessons; site = 169 lessons, 114 viz, 230 glossary, 123 code, all at full parity. Next genuine value needs OWNER STEER (Optimization module / 10th topic / lazy-load perf) or incremental polish.
   ★ Bayesian arc 100% done. Next genuine value needs OWNER STEER (new 10th topic / lazy-load perf) or is incremental polish. Possible self-chosen next deepening: an Optimization module (cross-cutting; gradient methods/convexity/constrained/KKT/2nd-order) — but confirm appetite.
   ...
   ✅ iter 597: **Connect newest IT lessons into the knowledge graph** (understandability/connections). +4 cross-topic prereq edges: it-differential-entropy →
   ps-random-variables-distributions + c-definite-integral-riemann; dl-autoencoders-vae → it-cross-entropy-kl + it-differential-entropy (VAE loss = reconstruction +
   KL). Was disconnected in the map. Verified: gate GREEN; headless connections both directions, map renders, errs=0. Prereq keys 49→50. SW →v538.
   ...
   it-channel-capacity (last IT lesson without a viz). Verified: gate GREEN (99 widgets); headless C(0)=1.000/C(0.5)=0.000/C(0.11)=0.500, kErr=0/errs=0; screenshot. SW →v531.
   ✅ iter 589: **Deepen Information Theory — Differential Entropy** (content/depth). 7th IT lesson it-differential-entropy (continuous entropy h=−∫f log f, can be
   negative, Gaussian ½log₂(2πeσ²) & max-entropy, KL/MI as invariant differences; full parity: Gaussian code→2.05 + 3 dd + 8 MCQ 2/2/2/2 + 5 cards + 3 ex + 3 hw),
   added to Foundations. Mobile spot-checked clean at 390px. Verified: gate GREEN (9 topics·165 lessons·118 code); headless nDD=3, code 2.05, kErr=0/errs=0. SW →v530. IT now 7 lessons.
   ...
   ✅ iter 581: **Information Theory module 4 — Source Coding** (content/depth). it-source-coding (prefix codes, Shannon source-coding theorem L≥H, Huffman,
   arithmetic coding, compression=prediction; full parity: Huffman code→1.75 + 3 dd + 8 MCQ 2/2/2/2 + 5 cards + 3 ex + 3 hw). Verified: gate GREEN (9 topics·162
   lessons·115 code); headless nDD=3, code 1.75, kErr=0/errs=0. SW →v521. IT now 4 lessons; modules 5–6 queued (channel capacity, info in ML).
   ...
   screenshot. SW →v515. QUEUED modules: cross-entropy & KL, mutual information, source coding (Shannon/Huffman), channel capacity, info in ML.
   ...
   ✅ iter 568: **Glossary +13 calculus terms** (content/reference; calc was thinnest at 17). Antiderivative, L'Hôpital, Newton's method, ODE, IVT/EVT,
   squeeze theorem, Euler's method, improper integral, etc. (calc 17→30, total 168→181). Feeds inline tooltips + glossary page + ⌘K. Verified: gate GREEN;
   headless new terms render (kErr=0), calc lesson inline tooltips work, errs=0. SW →v508.
   ✅ iter 567: **3 more 3rd dives: Poisson process · world models · scalable oversight** (content/depth; PS bucket now 0). ps-poisson (Poisson process/
   exponential), rl-connections-frontiers (world models/MuZero/Dreamer), l-safety-and-frontier (scalable oversight). Verified: gate GREEN; dump-dom each
   nDD=3, kErr=0/rawD=0/errs=0. SW →v507. (2dd ~13: calc 6, RL 4, LLM 2, PS 1 — wait PS now 1.)
   ...
   ✅ iter 564: **Hands-on code for RL/LLM/algorithms** (new functionality/code). rl-eligibility-traces (discounted return→3.349), l-what-is-a-language-model
   (softmax+greedy decode→0 0.665), a-amortized-analysis (doubling-array cost→31 1.94). Verified: gate RUNS them → 104 code-exercises, ALL GREEN; headless each
   Runs OK, errs=0. SW →v504. NOTE: code-injector guard must scope to the inserted block, not whole l.content (pre-existing x_{<t} math is runtime-escaped).
   ✅ iter 563: **3 more 3rd dives: matmul bottleneck · autodiff modes · TD-vs-MC** (content/depth). a-algorithms-for-ml (O(n³) matmul/GPUs),
   la-matrix-calculus-backprop (forward vs reverse autodiff), rl-td-learning (TD vs MC bias-variance). Verified: gate GREEN; dump-dom each nDD=3,
   kErr=0/rawD=0/errs=0. SW →v503. (2dd ~19; algo & LA buckets nearly cleared.)
   ...
   ✅ iter 554: **3 more 3rd deep-dives: QR · binary-search variants · convexity** (content/depth). la-orthonormal-gram-schmidt (QR & least squares),
   a-binary-search (lower/upper_bound), c-extrema-curve-sketching (convexity → global min). Verified: gate GREEN; dump-dom each nDD=3, kErr=0/rawD=0/errs=0. SW →v494. (2dd ~31.)
   ...
   ✅ iter 551: **Calculus numerical-methods code exercises** (new functionality/code; calc had ~no code). c-differentiation-rules (finite-difference
   derivative→6.001), c-area-volume (Riemann sum→0.338), c-linearization-lhopital (Newton's method→1.41421). Verified: gate RUNS them → 98 code-exercises,
   ALL GREEN; headless each Runs OK, errs=0. SW →v491. NOTE: toFixed() on a summed float can flip the last digit by summation order near a rounding
   boundary — use a rounding-stable precision.
   ✅ iter 550: **STEP-BACK: full 8-topic audit (clean) + 3 central calculus 3rd dives** (content/depth). Sweeps: 158 lessons errs=0/kErr=0/0 bad; 132 routes
   (incl. 8 cheatsheets, 8 placements, 95 labs) errs=0/kErr=0 (290 green). Coverage: 0 thin; dd dist 2dd:37 / 3dd:121 / 4+:0 (434). Reflection 540→550:
   owner streak bug fixed + whole-header FOUC fix + depth 2dd 52→37 + ML 6 viz/9 code. Ship: c-definite-integral-riemann (FTC), c-optimization (Lagrange
   multipliers→SVM/ridge), c-gradient-directional (gradient descent θ←θ−η∇L). Verified: gate GREEN; dump-dom nDD=3, kErr=0/rawD=0/errs=0. SW →v490.
   ...
   ✅ iter 537: **3 more original-topic 3rd deep-dives** (content/depth). c-intro-differential-equations (Euler's method/numerical ODEs), a-mst-union-find
   (MST applications: clustering/network/TSP-2-approx), ps-p-values (replication crisis & fixes). Verified: gate GREEN; dump-dom each nDD=3, kErr=0/rawD=0/errs=0. SW →v477.
   ✅ iter 536: **3 more ML code exercises** (new functionality/code; ML now 6/10 lessons with code). logistic-regression (sigmoid→0.50 1),
   linear-regression (MSE→1.00), ensembles (majority vote→1). Verified: gate RUNS them → 92 code-exercises (was 89), ALL GREEN; headless each Runs to
   matching output, errs=0. SW →v476.
   ✅ iter 535: **3rd deep-dives on 3 original-topic lessons** (content/depth; diversified off ML). c-improper-integrals (where they show up: Gamma/
   transforms/densities), ps-t-tests (paired/unpaired/Welch's), dl-practical-training-and-debugging (reading the loss curves). Verified: gate GREEN;
   dump-dom each nDD=3, kErr=0/rawD=0/errs=0. SW →v475.
   ...
   **ML pillar now integrated across every surface (lessons/dd/viz/code/prereqs/glossary/refs/search).**
   ...
   ✅ iter 526: **Interactive kNN decision-boundary viz** (visualizations). 91st widget ml-knn-viz: shades the plane by k-NN's prediction for 2 overlapping
   classes; k slider (1–31) shows jagged/overfit → smooth/oversmooth (bias-variance made visible); embedded in the kNN lesson + Lab. Verified: gate GREEN
   (91 widgets); headless Lab+lesson canvases hydrate, slider reclassifies, kErr=0/errs=0; screenshot eyeballed. SW →v466.
   ...
   ✅ iter 523: **ML topic — Model Selection & Cross-Validation (CAPSTONE, new module 6)** (content/new-topic build-out). train/val/test, k-fold CV,
   bias-variance diagnosis, grid/random search, metrics beyond accuracy, data leakage; 2 dd (never leak the test set/nested CV; accuracy trap / PR-ROC).
   8 mcq (2/2/2/2), 5 cards, 2 hw, 2 ex. **ML topic now COMPLETE: 6 modules / 10 lessons.** Verified: gate 8 topics/158 lessons ALL GREEN; headless course
   page 10, nDD=2, kErr=0/rawD=0/errs=0. SW →v463.  → Next: deepen ML (3rd dds / a clustering or boosting viz) or resume cross-topic depth lane.
   ...
   ✅ iter 520: **STEP-BACK: full 8-topic audit (clean) + Naive Bayes (finishes module 3)** (content/new-topic build-out). Sweeps: 154 lessons
   errs=0/kErr=0/0 bad; 109 routes errs=0/kErr=0 (263 green). Coverage: 0 thin; dd dist 2-dd:67 / 3-dd:87 / 4+:0 (395 total, every lesson >=2).
   Reflection: 510→520 launched + built the 8th topic (ML) to 7 lessons/3 modules. Ship: Naive Bayes (Bayes rule, independence, counting, Laplace;
   2 dd: wrong-assumption-works, generative-vs-discriminative). Verified: gate 8 topics/155 lessons ALL GREEN; headless nDD=2, kErr=0/rawD=0/errs=0. SW →v460.
   ✅ iter 519: **ML topic — Support Vector Machines (new module 3 "Kernel & Probabilistic Methods")** (content/new-topic build-out). Max-margin,
   support vectors, soft margin C, kernel trick (RBF), margin=regularization; 2 dd (kernel trick; why margin generalizes), 8 mcq (2/2/2/2), 5 cards,
   2 hw, 2 ex. Course now 3 modules/6 lessons. Verified: gate 8 topics/154 lessons ALL GREEN; headless course page 6, nDD=2, kErr=0/rawD=0/errs=0. SW →v459.
   ✅ iter 518: **ML topic — Regularization (finishes module 2)** (content/new-topic build-out). Ridge/L2 (closed form, collinearity), lasso/L1
   (sparsity, diamond geometry), λ via CV, Elastic Net; 2 dd (Bayesian prior / MAP; ridge vs lasso), 8 mcq (2/2/2/2), 5 cards, 2 hw, 2 ex. Course now
   2 modules/5 lessons. Verified: gate 8 topics/153 lessons ALL GREEN; headless nDD=2, kErr=0/rawD=0/errs=0. SW →v458.
   ✅ iter 517: **ML topic — Logistic Regression** (content/new-topic build-out). Linear score+sigmoid→probability, cross-entropy training, linear
   boundary, log-odds coefficients, =single neuron; 2 dd (cross-entropy vs squared error; linear classifier in disguise), 8 mcq (2/2/2/2), 5 cards,
   2 hw, 2 ex. Course now 2 modules/4 lessons. Verified: gate 8 topics/152 lessons ALL GREEN; headless nDD=2, kErr=0/rawD=0/errs=0. SW →v457.
   ...
   card on dashboard, course page, lesson + both dd + quiz render, kErr=0/rawD=0/errs=0; screenshots eyeballed. SW →v454.
   QUEUED: Decision Trees (finish module 1), then Linear/Logistic Regression, Regularization, SVM/kernels, Naive Bayes, k-Means, Ensembles, Model Selection/CV.
   OWNER: self-chosen 8th topic — rename/redirect/kill welcome (additive file, reversible); else loop keeps building it module-by-module.
   ...
   ✅ iter 511: **Memoize the ⌘K search index** (performance; anti-monotony off content). Index (incl. regex over all 377 dives) was rebuilt every
   palette open though it's pure over static data → now built once & cached (_searchIdx). Verified: gate GREEN; headless repeated cached queries return
   correct results incl. dd hits, errs=0; mobile 390px dashboard+lesson eyeballed clean (no fix needed). SW →v451.
   ...
   ✅ iter 508: **3 more THIRD deep-dives on the hardest concepts** (content/depth; alternating with 507). +3 dd (371→374; 78 at 3 dd):
   la-gradients-jacobians (forward vs reverse-mode autodiff), a-union-find-range (the segment tree), rl-policy-iteration (generalized policy
   iteration). Verified: gate GREEN; dump-dom each nDD=3, kErr=0, rawDollar=0; smoke bad=none. SW →v448.
   ...
   ✅ iter 504: **3 more THIRD deep-dives on the hardest concepts** (content/depth; alternating with 503's search). +3 dd (365→368; 72 at 3 dd):
   la-projection-least-squares (don't solve the normal equations — QR/SVD), rl-trpo-ppo (GAE), a-network-flow (flow in disguise / reductions).
   Verified: gate GREEN; dump-dom each nDD=3, kErr=0, rawDollar=0; smoke bad=none. SW →v444.
   ...
   ✅ iter 501: **UI/UX — "In this lesson" deeper-dives badge counts + opens all** (pivot off the depth lane for anti-monotony). Badge now reads
   "🧩 N deeper dives" and one click opens every dd (was: generic label, opened only the first) — surfacing the 2–3 dd/lesson depth. Verified:
   gate GREEN; headless la-svd badge "3 deeper dives", click openBefore=0→openAfter=3, kErr=0; 14-route smoke errs=0/kErr=0/bad=none. SW →v441.
   ...
   ✅ iter 498: **3 more THIRD deep-dives on the hardest concepts** (content/depth). +3 dd (353→356; 60 lessons now at 3 dd):
   ps-expectation-variance (indicator method), l-multihead-and-causal-masking (MQA/GQA KV-cache), c-area-volume (arc length & surface area).
   Append-dd inject w/ full guards. Verified: gate GREEN; dump-dom each nDD=3, kErr=0, rawDollar=0; smoke bad=none. SW →v438.
   ✅ iter 497: **3 more THIRD deep-dives on the hardest concepts** (content/depth). +3 dd (350→353; 57 lessons now at 3 dd):
   a-divide-and-conquer (paradigm + parallelism), ps-covariance-correlation (the covariance matrix), la-matrices-as-transformations
   (non-square = dimension change). Append-dd inject w/ full guards. Verified: gate GREEN; dump-dom each nDD=3, kErr=0, rawDollar=0; smoke bad=none. SW →v437.
   ✅ iter 496: **3 more THIRD deep-dives on the hardest concepts** (content/depth). +3 dd (347→350; 54 lessons now at 3 dd):
   l-what-is-a-language-model (next-token prediction subsumes everything), ps-point-estimation (MSE=bias²+variance), dl-ml-recap-and-the-learning-problem
   (no free lunch / inductive bias). Append-dd inject w/ full guards. Verified: gate GREEN; dump-dom each nDD=3, kErr=0, rawDollar=0; smoke bad=none. SW →v436.
   ✅ iter 495: **3 more THIRD deep-dives on the hardest concepts** (content/depth). +3 dd (344→347; 51 lessons now at 3 dd):
   dl-autoencoders-vae (generative trilemma), a-graph-representations-traversal (what BFS/DFS unlock), rl-imitation (inverse RL).
   Append-dd inject w/ full guards. Verified: gate GREEN; dump-dom each nDD=3, kErr=0, rawDollar=0; smoke bad=none. SW →v435.
   ✅ iter 494: **3 more THIRD deep-dives on the hardest concepts** (content/depth). +3 dd (341→344; 48 lessons now at 3 dd):
   rl-monte-carlo (exploring starts / GLIE), la-dot-product-norms (L1/L2/L∞ norms), a-trees-heaps (rotations / balanced-tree family).
   Append-dd inject w/ full guards. Verified: gate GREEN; dump-dom each nDD=3, kErr=0, rawDollar=0; smoke bad=none. SW →v434.
   ✅ iter 493: **3 more THIRD deep-dives on the hardest concepts** (content/depth). +3 dd (338→341; 45 lessons now at 3 dd):
   dl-activation-functions (GELU/SiLU zoo), ps-confidence-intervals (CI/test duality), l-tokenization-bpe (beyond BPE / byte-level).
   Append-dd inject w/ full guards. Verified: gate GREEN; dump-dom each nDD=3, kErr=0, rawDollar=0; smoke bad=none. SW →v433.
   ✅ iter 492: **3 more THIRD deep-dives on the hardest concepts** (content/depth). +3 dd (335→338; 42 lessons now at 3 dd):
   la-inverse-and-systems (LU decomposition), l-rag-and-tools (retrieval = k-NN / vector DB / chunking), rl-value-iteration (curse of
   dimensionality → deep RL). Append-dd inject w/ full guards. Verified: gate GREEN; dump-dom each nDD=3, kErr=0, rawDollar=0; smoke bad=none. SW →v432.
   ✅ iter 491: **3 more THIRD deep-dives on the hardest concepts** (content/depth). +3 dd (332→335; 39 lessons now at 3 dd):
   dl-diffusion-models (forward/reverse + score matching), la-basis-dimension (functions are vectors / Fourier), a-np-completeness (coping
   with NP-hardness). Append-dd inject w/ full guards. Verified: gate GREEN; dump-dom each nDD=3, kErr=0, rawDollar=0; smoke bad=none. SW →v431.
   ✅ iter 490: **STEP-BACK: full audit (clean) + 3 more third deep-dives** (content/depth). Sweeps: 148 lessons errs=0/kErr=0/0 bad; 107
   routes errs=0/kErr=0 (255 green). Coverage: 0 thin; dd dist 2-dd:115 / 3-dd:36 / 4+:0 (every lesson >=2; 332 dd total). Reflection
   (480-489): runway complete then a clean 3rd-dd lane on hardest concepts, quality held (verify vs dds AND examples). Ship: ps-normal-distribution
   (max-entropy), dl-gans (likelihood-free), l-prompting-and-in-context-learning (induction heads). SW →v430. ~7-10 selective 3rd-dd iters left then pivot.
   ✅ iter 489: **3 more THIRD deep-dives on the hardest concepts** (content/depth). +3 dd (326→329; 33 lessons now at 3 dd):
   la-symmetric-spectral (positive definiteness), a-shortest-paths-topo (A*), rl-exploration (Thompson sampling). Append-dd inject w/ full
   guards. Verified: gate GREEN; dump-dom each nDD=3, kErr=0, rawDollar=0; smoke bad=none. SW →v429.
   ✅ iter 488: **3 more THIRD deep-dives on the hardest concepts** (content/depth). +3 dd (323→326; 30 lessons now at 3 dd):
   c-convexity (why DL is non-convex but works), la-span-independence (independence = rank computation), a-hash-tables (open addressing vs
   chaining). Append-dd inject w/ full guards. Verified: gate GREEN; dump-dom each nDD=3, kErr=0, rawDollar=0; smoke bad=none. SW →v428.
   ✅ iter 487: **3 more THIRD deep-dives on the hardest concepts** (content/depth). +3 dd (320→323; 27 lessons now at 3 dd):
   dl-pooling-and-cnn-architectures (residual connections / depth wall), ps-hypothesis-testing-logic (statistical vs practical significance),
   a-comparison-sorts (stability & in-place). Append-dd inject w/ full guards. Verified: gate GREEN; dump-dom each nDD=3, kErr=0, rawDollar=0; smoke bad=none. SW →v427.
   ✅ iter 486: **3 more THIRD deep-dives on the hardest concepts** (content/depth). +3 dd (317→320; 24 lessons now at 3 dd):
   a-dynamic-programming (the two DP conditions), dl-embeddings-and-tokenization (contextual vs static), l-self-attention (learned
   content-based routing). Append-dd inject w/ full guards. Verified: gate GREEN; dump-dom each nDD=3, kErr=0, rawDollar=0; smoke bad=none. SW →v426.
   ✅ iter 485: **3 more THIRD deep-dives on the hardest concepts** (content/depth). +3 dd (314→317; 21 lessons now at 3 dd):
   dl-rnn-lstm-gru (BPTT), rl-sarsa-qlearning (maximization bias / Double Q), l-embeddings-and-prediction-head (the logit lens).
   Append-dd inject w/ full guards. Verified: gate GREEN; dump-dom each nDD=3, kErr=0, rawDollar=0; smoke bad=none. SW →v425.
   ✅ iter 484: **3 more THIRD deep-dives on the hardest concepts** (content/depth). +3 dd (311→314; 18 lessons now at 3 dd):
   dl-the-artificial-neuron-and-mlp (no nonlinearity → depth collapses), la-determinants (computing det: cofactor n! vs LU O(n³)),
   rl-mdp-formalism (POMDPs / when Markov fails). Append-dd inject w/ full guards. Verified: gate GREEN; dump-dom each nDD=3, kErr=0, rawDollar=0; smoke bad=none. SW →v424.
   ✅ iter 483: **3 more THIRD deep-dives on the hardest concepts** (content/depth). +3 dd (308→311; 15 lessons now at 3 dd):
   dl-transformer-architecture (parallelism replaced RNNs), c-gradient-descent-convergence (convergence rate / preconditioning),
   ps-conditional-independence-bayes (odds form / likelihood ratio). Append-dd inject w/ full guards. Verified: gate GREEN; dump-dom each nDD=3, kErr=0, rawDollar=0; smoke bad=none. SW →v423.
   ✅ iter 482: **3 more THIRD deep-dives on the hardest concepts** (content/depth). +3 dd (305→308; 12 lessons now at 3 dd):
   la-matrix-multiplication (four views of AB / outer products), c-multivariable-optimization (saddles dominate high-D), dl-convolution-operation
   (receptive field grows with depth). Append-dd inject w/ full guards. Verified: gate GREEN; dump-dom each nDD=3, kErr=0, rawDollar=0; smoke bad=none. SW →v422.
   ...
   Append-second-dd inject w/ full guards. Verified: gate GREEN; dump-dom each nDD=2, kErr=0, rawDollar=0; smoke bad=none. SW →v418.
   ✅ iter 477: **3 more second deep-dives on the hardest lessons** (content/depth). +3 dd (290→293; 141 lessons have 2):
   la-low-rank-pca (Eckart-Young best rank-k), l-positional-encoding (absolute vs relative / length extrapolation), c-extrema-curve-sketching
   (Fermat's theorem). Append-second-dd inject w/ full guards. Verified: gate GREEN; dump-dom each nDD=2, kErr=0, rawDollar=0; smoke bad=none. SW →v417.
   ✅ iter 476: **3 more second deep-dives on the hardest lessons** (content/depth). +3 dd (287→290; 138 lessons have 2):
   c-integration-techniques (partial fractions), l-optimization-and-stability (mixed precision / loss scaling), rl-actor-critic (A2C/A3C
   parallel actors). Append-second-dd inject w/ full guards. Verified: gate GREEN; dump-dom each nDD=2, kErr=0, rawDollar=0; smoke bad=none. SW →v416.
   ✅ iter 475: **3 more second deep-dives on the hardest lessons** (content/depth). +3 dd (284→287; 135 lessons have 2):
   c-lagrange-multipliers (∇f∥∇g tangency), a-mst-union-find (Prim vs Kruskal), rl-value-approximation (features / tile coding for linear
   VFA). Append-second-dd inject w/ full guards. Verified: gate GREEN; dump-dom each nDD=2, kErr=0, rawDollar=0; smoke bad=none. SW →v415.
   ✅ iter 474: **3 more second deep-dives on the hardest lessons** (content/depth). +3 dd (281→284; 132 lessons have 2):
   c-differentiation-rules (logarithmic differentiation), rl-practical-rl (seeds/variance/reproducibility), la-matrix-calculus-backprop
   (vector-Jacobian products). Append-second-dd inject w/ full guards. Verified: gate GREEN; dump-dom each nDD=2, kErr=0, rawDollar=0; smoke bad=none. SW →v414.
   ✅ iter 473: **3 more second deep-dives on the hardest lessons** (content/depth). +3 dd (278→281; 129 lessons have 2):
   l-multihead-and-causal-masking (how the causal mask works), a-union-find-range (Fenwick low-bit trick), c-limits-intuition (limits
   at/of infinity / asymptotes). Append-second-dd inject w/ full guards. Verified: gate GREEN; dump-dom each nDD=2, kErr=0, rawDollar=0; smoke bad=none. SW →v413.
   ✅ iter 472: **3 more second deep-dives on the hardest lessons** (content/depth). +3 dd (275→278; 126 lessons have 2):
   rl-trpo-ppo (TRPO trust region / KL bound), l-finetuning-and-instruction-tuning (catastrophic forgetting), c-continuity (formal def +
   3 discontinuity types). Append-second-dd inject w/ full guards. Verified: gate GREEN; dump-dom each nDD=2, kErr=0, rawDollar=0; smoke bad=none. SW →v412.
   ✅ iter 471: **3 more second deep-dives on the hardest lessons** (content/depth). +3 dd (272→275; 123 lessons have 2):
   l-inference-efficiency (speculative decoding), rl-connections-frontiers (self-play / AlphaZero), c-definite-integral-riemann (signed
   area). Append-second-dd inject w/ full guards. Verified: gate GREEN; dump-dom each nDD=2, kErr=0, rawDollar=0; smoke bad=none. SW →v411.
   ✅ iter 470: **STEP-BACK: full audit (clean) + 3 second deep-dives** (content/depth). Sweeps: 148 lessons errs=0/kErr=0/0 bad; 107 routes
   errs=0/kErr=0 (255 green). Coverage: 0 thin (443 examples, 272 dd [120 lessons 2+, 28 at 1], 90 code, 151 glossary, 89 viz). Reflection
   (460–469): 9 dd batches + iter 463 streak-bug fix, quality holding, no red gates; 28 lessons left at 1 dd. Ship: +3 2nd-dd rl-td-learning
   (TD update = EMA / α), l-safety-and-frontier (specification gaming / reward hacking), c-fundamental-theorem (net change). SW →v410.
   Verified: gate GREEN; dump-dom each nDD=2, kErr=0, rawDollar=0; smoke bad=none.
   ✅ iter 469: **3 more second deep-dives on the hardest lessons** (content/depth). +3 dd (266→269; 117 lessons have 2):
   a-arrays-lists-stacks-queues (cache locality), la-orthonormal-gram-schmidt (GS numerical fragility / Householder), c-improper-integrals
   (convergence tests / p-integral). Append-second-dd inject w/ full guards. Verified: gate GREEN; dump-dom each nDD=2, kErr=0, rawDollar=0; smoke bad=none. SW →v409.
   ✅ iter 468: **3 more second deep-dives on the hardest lessons** (content/depth). +3 dd (263→266; 114 lessons have 2):
   la-diagonalization (spectral theorem), dl-autoencoders-vae (why plain AE can't generate / KL fix), c-chain-rule (multivariable chain
   rule = sum over paths). Append-second-dd inject w/ full guards. Verified: gate GREEN; dump-dom each nDD=2, kErr=0, rawDollar=0; smoke bad=none. SW →v408.
   ✅ iter 467: **3 more second deep-dives on the hardest lessons** (content/depth). +3 dd (260→263; 111 lessons have 2):
   l-hallucination-and-evaluation (how to evaluate an LLM / why it's hard), rl-offline (conservatism: policy constraint + CQL),
   a-recurrences-master-theorem (when the Master Theorem fails). Append-second-dd inject w/ full guards. Verified: gate GREEN; dump-dom each nDD=2, kErr=0, rawDollar=0; smoke bad=none. SW →v407.
   ✅ iter 466: **3 more second deep-dives on the hardest lessons** (content/depth). +3 dd (257→260; 108 lessons have 2):
   c-gradient-directional (directional derivative ∇f·û), a-correctness-invariants (invariants vs variants / termination),
   dl-pretraining-and-finetuning-paradigm (self-supervision). Append-second-dd inject w/ full guards. Verified: gate GREEN; dump-dom each nDD=2, kErr=0, rawDollar=0; smoke bad=none. SW →v406.
   ✅ iter 465: **3 more second deep-dives on the hardest lessons** (content/depth). +3 dd (254→257; 105 lessons have 2):
   dl-initialization-and-vanishing-gradients (Xavier/He variance-preserving init), rl-model-based (Dyna), c-antiderivatives (u-substitution
   / reverse chain rule). Append-second-dd inject w/ full guards. Verified: gate GREEN; dump-dom each nDD=2, kErr=0, rawDollar=0; smoke bad=none. SW →v405.
   ✅ iter 464: **3 more second deep-dives on the hardest lessons** (content/depth). +3 dd (251→254; 102 lessons have 2):
   c-implicit-related-rates (implicit differentiation), a-network-flow (residual graphs / augmenting paths), l-peft-lora (LoRA merges away,
   zero inference overhead). Append-second-dd inject w/ full guards. Verified: gate GREEN; dump-dom each nDD=2, kErr=0, rawDollar=0; smoke bad=none. SW →v404.
   ...
   ✅ iter 461: **3 more second deep-dives on the hardest lessons** (content/depth). +3 dd (245→248; 96 lessons have 2):
   la-vectors-operations (linear combinations = the one operation), ps-p-values (p-hacking & multiple comparisons), a-algorithms-for-ml
   (vectorization). Append-second-dd inject w/ full guards. Verified: gate GREEN; dump-dom each nDD=2, kErr=0, rawDollar=0; smoke bad=none. SW →v401.
   ✅ iter 460: **STEP-BACK: full audit (clean) + 3 second deep-dives** (content/depth). Sweeps: 148 lessons errs=0/kErr=0/0 bad; 107 routes
   errs=0/kErr=0 (255 green). Coverage: 0 thin (443 examples, 245 dd [93 lessons 2+, 55 at 1], 90 code, 151 glossary, 89 viz). Reflection
   (451–459): 9 straight 2nd-dd batches, quality holding, no red gates; ~55 lessons left at 1 dd. Ship: +3 2nd-dd ps-sample-spaces-events
   (axioms + inclusion-exclusion), rl-policies-values (V vs Q / model-free), c-functions-and-graphs (composition & inverses). SW →v400.
   Verified: gate GREEN; dump-dom each nDD=2, kErr=0, rawDollar=0; smoke bad=none.
   ✅ iter 459: **3 more second deep-dives on the hardest lessons** (content/depth). +3 dd (239→242; 90 lessons have 2):
   ps-errors-and-power (statistical power 1−β), c-derivatives-special-functions (trig-derivative 4-cycle / Euler), l-transformer-block
   (pre-LN vs post-LN). Append-second-dd inject w/ full guards. Verified: gate GREEN; dump-dom each nDD=2, kErr=0, rawDollar=0; smoke bad=none. SW →v399.
   ✅ iter 458: **3 more second deep-dives on the hardest lessons** (content/depth). +3 dd (236→239; 87 lessons have 2):
   dl-dropout-and-normalization (dropout as implicit ensemble), dl-ml-recap-and-the-learning-problem (empirical risk & generalization gap),
   rl-policy-iteration (policy improvement theorem). Append-second-dd inject w/ full guards. Verified: gate GREEN; dump-dom each nDD=2, kErr=0, rawDollar=0; smoke bad=none. SW →v398.
   ...
   ✅ iter 455: **3 more second deep-dives on the hardest lessons** (content/depth). +3 dd (227→230; 78 lessons have 2):
   rl-what-is-rl (the reward hypothesis), dl-learning-rate-schedules-and-tuning (linear scaling rule LR∝batch), c-area-volume (disks vs
   shells). Append-second-dd inject w/ full guards. Verified: gate GREEN; dump-dom each nDD=2, kErr=0, rawDollar=0; smoke bad=none. SW →v395.
   ✅ iter 454: **3 more second deep-dives on the hardest lessons** (content/depth). +3 dd (224→227; 75 lessons have 2):
   l-tokenization-bpe (vocab-size tradeoff), ps-t-tests (t→normal as df grows), la-gradients-jacobians (chain rule = Jacobian product →
   backprop). Append-second-dd inject w/ full guards. Verified: gate GREEN; dump-dom each nDD=2, kErr=0, rawDollar=0; smoke bad=none. SW →v394.
   ✅ iter 453: **3 more second deep-dives on the hardest lessons** (content/depth). +3 dd (221→224; 72 lessons have 2):
   c-derivative-definition (differentiable⟹continuous, not reverse), a-backtracking-branch-bound (branch & bound / optimality pruning),
   dl-practical-training-and-debugging (overfit one batch first). Append-second-dd inject w/ full guards. Verified: gate GREEN; dump-dom each nDD=2, kErr=0, rawDollar=0; smoke bad=none. SW →v393.
   ✅ iter 452: **3 more second deep-dives on the hardest lessons** (content/depth). +3 dd (218→221; 69 lessons have 2):
   la-matrices-as-transformations (what makes a transformation linear), ps-joint-distributions (marginalization), rl-eligibility-traces
   (the eligibility trace / backward view). Append-second-dd inject w/ full guards. Verified: gate GREEN; dump-dom each nDD=2, kErr=0, rawDollar=0; smoke bad=none. SW →v392.
   ✅ iter 451: **3 more second deep-dives on the hardest lessons** (content/depth). +3 dd (215→218; 66 lessons have 2):
   c-partial-derivatives (mixed partials commute / Clairaut → symmetric Hessian), dl-diffusion-models (why many small steps),
   l-rag-and-tools (RAG vs finetuning). Append-second-dd inject w/ full guards. Verified: gate GREEN; dump-dom each nDD=2, kErr=0, rawDollar=0; smoke bad=none. SW →v391.
   ✅ iter 450: **STEP-BACK: full audit (clean) + 3 second deep-dives** (content/depth). Sweeps: 148 lessons errs=0/kErr=0/0 bad; 107 routes
   errs=0/kErr=0 (255 green). Coverage: 0 thin (443 examples, 215 dd [85 at 1 dd, 63 at 2], 90 code, 151 glossary, 89 viz). Reflection
   (441–449): depth runway + 1 viz + owner work (glossary, prerender.js, gitignore). Ship: +3 2nd-dd (212→215; 63 lessons have 2)
   la-span-independence (dimension invariant), ps-geometric-waiting (E=1/p), a-approximation-randomized (ratio / Las Vegas vs Monte Carlo).
   Verified: gate GREEN; dump-dom each nDD=2, kErr=0, rawDollar=0; smoke bad=none. SW →v390. Main remaining work: 2nd-dd on the 85 single-dd lessons.
   ✅ iter 449: **3 more second deep-dives on the hardest lessons** (content/depth). +3 dd (209→212; 62 lessons have 2):
   a-asymptotic-analysis (Big-O of which case / O,Θ,Ω), ps-sampling-distributions (statistic vs parameter), rl-imitation (DAgger).
   Append-second-dd inject w/ full guards. Verified: gate GREEN; dump-dom each nDD=2, kErr=0, rawDollar=0; smoke bad=none. SW →v389.
   ...
   dump-dom each nDD=2, kErr=0, rawDollar=0; smoke bad=none. SW →v385.
   ✅ iter 444: **3 more second deep-dives — the 200th** (content/depth — owner's #1). +3 dd (197→200; 50 lessons have 2):
   c-optimization (closed-interval method / endpoints), dl-embeddings-and-tokenization (why tokenization fumbles spelling/digits),
   rl-monte-carlo (first-visit vs every-visit; needs complete episodes). Append-second-dd inject w/ full guards. Verified: gate GREEN
   (200 dd); dump-dom each nDD=2, kErr=0, rawDollar=0; smoke bad=none. SW →v384.
   ...
   Also removed stray gitignored __*.html scratch files left at repo root by interrupted iters.
   ...
   rawDollar=0; smoke bad=none. SW →v377.
   ...
   kErr=0, rawDollar=0; smoke bad=none. SW →v375.
   ✅ iter 434: **3 more worked examples across DL/RL/LLM** (examples). +3 (431→434): dl-transfer-learning (small LR / catastrophic
   forgetting), rl-trpo-ppo (importance ratio → multi-epoch reuse; r=1.5 clipped 1.2), l-inference-efficiency (prefill vs decode).
   Node-verified; round-trip-guarded. Verified: gate GREEN (434); dump-dom katex 35/107/70, kErr=0, rawDollar=0; smoke bad=none. SW →v374.
