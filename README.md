...
examples · 47 interactive visualizations**, each lesson with rendered math, step-by-step worked examples, and
...
- **Code Playground** — run real **Python** (via Pyodide, incl. numpy) and **JavaScript** in the browser; **8 lessons now embed a runnable, self-checking code exercise** right in the lecture (merge sort, BFS, greedy activity-selection, binary search, bubble sort, bottom-up Fibonacci, a neuron's forward pass, backprop) — edit, **Run**, and get an instant ✓/✗ against the expected output. Embed anywhere with `<div data-code="javascript" data-expected="...">code</div>`.
...
- **Daily goal + activity heatmap + streak freezes**; a **recent-test performance trend** on the Progress page (your last tests as score bars with running average & best, so you can watch your exam results climb); **Cheatsheets** (print/PDF); **Placement diagnostic** to skip ahead; **Progress export/import**; **Library** of curated references.
- **Gamification** — XP (with a floating "+XP" reward burst and an animated level ring), 10 named levels (Novice → Polymath) shown as a ranks ladder, daily streak, **55 achievements** (grouped into themed categories — Lessons, Quizzes, Streaks, Flashcards, Mastery, Levels, Exploration — each with its own unlocked count) with a collection-progress bar, and confetti celebrations on a level-up and the moment you **hit your daily XP goal** (it fires the instant you cross it — mid-quiz, mid-review — not as an afterthought, rewarding the daily habit precisely when it happens). The Progress page's stats cascade-count up from zero when you land on it — a small "look how far you've come" flourish (reduced-motion safe). Achievements reward the behaviors that actually build knowledge — trying a lecture's no-stakes **Quick Check** (and acing one), exploring the visualizations — and there are long-haul "endgame" capstones — matched to the now-complete codex — to chase across months of study: complete all 100+ lessons, answer 5,000 questions correctly, master 100 concepts, hold a 365-day streak, earn 100,000 XP, ace a flawless 40-question exam, and open every visualization.
