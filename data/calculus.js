      ...
      "lessons": [
        {
          ...
          "examples": [
            ...
            },
            {
              ...
              "body": "Let $f(x) = \\dfrac{1}{x}$ and $g(x) = x^2 - 9$. Find $(f \\circ g)(x)$, its domain, and evaluate it at $x = 4$.",
              "solution": "<strong>Compose inside-out.</strong> $(f \\circ g)(x) = f(g(x)) = f(x^2 - 9) = \\dfrac{1}{x^2 - 9}$ — feed $g$'s output into $f$. Order matters: $(g \\circ f)(x) = \\dfrac{1}{x^2} - 9$ is a different function.\n<strong>Find the domain.</strong> The inner $g$ accepts every real $x$, but $f$ rejects the input $0$ — so we must exclude every $x$ with $g(x)=0$: $x^2 - 9 = 0$ gives $x = \\pm 3$. The domain is $\\mathbb{R} \\setminus \\{-3, 3\\}$ — this time composition punches <em>two isolated holes</em> rather than cutting away an interval (contrast the square-root example above, where the restriction was $x \\ge 4$).\n<strong>Evaluate.</strong> $(f \\circ g)(4) = \\dfrac{1}{16 - 9} = \\dfrac{1}{7}$.\n<strong>The aha.</strong> Composition chains functions — the output of one becomes the input of the next — and the composite's domain is every $x$ that survives the <em>whole</em> chain: legal for $g$, and producing something legal for $f$. Which shape the restriction takes (an interval, isolated holes, a half-line) depends on which rule the outer function enforces."
            }
          ]
        },
        {
          ...
          "examples": [
            ...
            {
              "title": "The squeeze theorem: trapping a wild function",
              "body": "Evaluate $\\lim_{x \\to 0} x^2 \\sin\\!\\left(\\tfrac{1}{x}\\right)$ — even though $\\sin(1/x)$ oscillates infinitely fast near 0.",
              "solution": "<strong>Bounded times tiny.</strong> Direct substitution fails: $\\sin(1/x)$ has no limit at 0 (it oscillates between $-1$ and $1$ forever). But it is <em>bounded</em>: $-1 \\le \\sin(1/x) \\le 1$ for all $x \\ne 0$.\n<strong>Squeeze it.</strong> Multiplying that bound by $x^2 \\ge 0$ preserves the inequalities: $-x^2 \\le x^2\\sin(1/x) \\le x^2$. As $x \\to 0$, both the lower bound $-x^2$ and the upper bound $x^2$ go to $0$, trapping the function between two things heading to the same place:\n$$\\lim_{x \\to 0} x^2 \\sin\\!\\left(\\tfrac{1}{x}\\right) = 0.$$\n<strong>Why it works.</strong> The <b>squeeze (sandwich) theorem</b>: if $g(x) \\le f(x) \\le h(x)$ near $a$ and $g, h \\to L$, then $f \\to L$ — you never evaluate $f$ directly, only bound it between two functions that agree in the limit.\n<strong>The aha.</strong> A limit does not require the function to be tame — only to be <em>trapped</em>. Squeeze converts \"I can't evaluate this\" into \"I can bound this,\" which is exactly how $\\lim_{x\\to 0}\\tfrac{\\sin x}{x} = 1$ and many other stubborn limits are pinned down."
            }
          ]
        },
        {
      ...
      "lessons": [
        {
          ...
          "examples": [
            ...
            {
              "title": "When the derivative doesn't exist: a vertical tangent",
              "body": "The corner of $|x|$ is one failure mode; here is a different one. Show that $f(x) = x^{1/3}$ (the cube root) is continuous at $x = 0$ yet has no derivative there — and describe what its graph does instead.",
              "solution": "<strong>Continuity is fine.</strong> $\\lim_{x\\to 0} x^{1/3} = 0 = f(0)$: the cube root is defined for all reals (negative inputs give negative roots) and passes through the origin without a break.\n<strong>Set up the difference quotient.</strong> At $x=0$: $\\dfrac{f(0+h)-f(0)}{h} = \\dfrac{h^{1/3}}{h} = h^{-2/3} = \\dfrac{1}{h^{2/3}}$.\n<strong>Take the limit — from both sides.</strong> For $h>0$, $h^{2/3}\\to 0^+$ so the quotient blows up to $+\\infty$. For $h<0$, $h^{1/3}$ is negative and $h$ is negative, so the quotient is again positive: $h^{-2/3} = 1/(h^{2/3})$ with $h^{2/3}=(|h|^{1/3})^2>0$, and it blows up to $+\\infty$ too (check: $h=\\pm 0.001$ both give quotient $100$).\n<strong>The verdict.</strong> Unlike the corner, the two sides <em>agree</em> — but they agree on $+\\infty$, which is not a real number, so $f'(0)$ does not exist. The graph has a <b>vertical tangent</b>: it passes through the origin standing straight up.\n<strong>The lesson.</strong> Differentiability can fail in more than one way: a corner (one-sided slopes disagree, like $|x|$ and ReLU), a vertical tangent (slopes agree but are infinite, like $x^{1/3}$), or wild oscillation. Each failure is a different answer to \"what did the difference quotient do?\""
            }
          ]
        },
        {
          ...
          "examples": [
            ...
            {
              "title": "The product rule, verified two ways",
              "body": "Differentiate $f(x) = \\sqrt{x}\\,(x^2 - 4)$ twice: once with the product rule, and once by expanding into powers first. Confirm the answers agree.",
              "solution": "<strong>Route 1 — product rule.</strong> With $u = x^{1/2}$ (so $u' = \\tfrac12 x^{-1/2}$) and $v = x^2 - 4$ (so $v' = 2x$):\n$$f'(x) = u'v + uv' = \\frac{x^2-4}{2\\sqrt{x}} + 2x\\sqrt{x}.$$\nPut over a common denominator: $\\dfrac{x^2 - 4 + 4x^2}{2\\sqrt{x}} = \\dfrac{5x^2 - 4}{2\\sqrt{x}}$.\n<strong>Route 2 — expand first.</strong> $f(x) = x^{5/2} - 4x^{1/2}$, so by the power rule\n$$f'(x) = \\tfrac{5}{2}x^{3/2} - 2x^{-1/2} = \\frac{5x^2 - 4}{2\\sqrt{x}}.$$\nIdentical — spot-check at $x=4$: both give $\\dfrac{5\\cdot 16 - 4}{2\\cdot 2} = 19$.\n<strong>The takeaway.</strong> The product rule is $u'v + uv'$, never $u'v'$ — each factor is differentiated in turn while the other stands still. And as the lesson's strategy says: when a product <em>can</em> be expanded into plain powers, expanding first is usually the shorter, safer route; the product rule earns its keep when the factors don't merge (that is exactly where it becomes indispensable once transcendental functions arrive two lessons from now)."
            }
          ]
        },
        {
    ...
    },
    {
      ...
      "lessons": [
        ...
        },
        {
          ...
          "homework": [
            {
              "prompt": "Let $f(x) = x^4 - 8x^2$. Find all critical points, classify each as local max/min/neither, find intervals of increase/decrease, find concavity intervals, and identify all inflection points.",
              "hint": "Factor $f'$ and $f''$ completely. The symmetry of this even function is a useful cross-check: whatever happens at $x=c$ must mirror at $x=-c$.",
              "solution": "$f'(x)=4x^3-16x=4x(x^2-4)=4x(x-2)(x+2)$, so critical points are $x=-2, 0, 2$. Sign of $f'$: at $x=-3$: $4(-3)(5)=-60<0$ (decreasing); at $x=-1$: $4(-1)(-3)=12>0$ (increasing); at $x=1$: $4(1)(-3)=-12<0$ (decreasing); at $x=3$: $60>0$ (increasing). So $f'$ goes $-\\to+$ at $x=-2$ (local min, $f(-2)=16-32=-16$), $+\\to-$ at $x=0$ (local max, $f(0)=0$), and $-\\to+$ at $x=2$ (local min, $f(2)=-16$). Increase on $(-2,0)\\cup(2,\\infty)$; decrease on $(-\\infty,-2)\\cup(0,2)$. Concavity: $f''(x)=12x^2-16$, zero at $x=\\pm\\frac{2}{\\sqrt3}\\approx\\pm1.155$. $f''>0$ for $|x|>\\frac{2}{\\sqrt3}$ (concave up) and $f''<0$ between (concave down), so concavity changes at both — genuine inflection points at $\\left(\\pm\\frac{2}{\\sqrt3},\\,-\\frac{80}{9}\\right)$ (since $x^2=\\frac43$ gives $f=\\frac{16}{9}-\\frac{32}{3}=-\\frac{80}{9}\\approx-8.89$). Cross-check: the Second Derivative Test agrees — $f''(0)=-16<0$ (max), $f''(\\pm2)=32>0$ (mins) — and everything mirrors in $x\\to-x$, as an even function must."
            },
            {
        ...
        },
        {
          ...
          "examples": [
            ...
            {
              "title": "Linear approximation: the tangent line as a calculator",
              "body": "Estimate $\\sqrt{4.1}$ without a calculator, using the linearization of $f(x) = \\sqrt{x}$ at $a = 4$.",
              "solution": "<strong>The tangent-line formula.</strong> Near $a$, a differentiable function is well approximated by its tangent line: $f(x) \\approx f(a) + f'(a)(x - a)$. For $f(x) = \\sqrt{x}$, $f'(x) = \\tfrac{1}{2\\sqrt{x}}$.\n<strong>Evaluate at $a = 4$.</strong> $f(4) = 2$ and $f'(4) = \\tfrac{1}{2\\cdot 2} = 0.25$, so the linearization is $L(x) = 2 + 0.25\\,(x - 4)$. Then\n$$\\sqrt{4.1} \\approx L(4.1) = 2 + 0.25(0.1) = 2.025.$$\nThe true value is $2.0248\\ldots$ — the estimate is off by under $0.0002$.\n<strong>Why it is so accurate.</strong> The error of a linear approximation is governed by the <em>second</em> derivative and shrinks like $(x - a)^2$: move only $0.1$ from the anchor and the squared term is $0.01$, scaled by a small curvature. Stay close to $a$ and the tangent line is nearly the curve.\n<strong>The aha.</strong> \"Differentiable\" means \"locally linear\" — zoom in far enough on any smooth curve and it looks straight. Linearization just uses that straight line as a stand-in, the same idea behind Newton's method, error propagation, and every \"small-signal\" approximation in science."
            }
          ]
        }
      ]
      ...
      "lessons": [
        ...
        },
        {
          ...
          "examples": [
            ...
            {
              "title": "Left, right, midpoint: which Riemann sum wins",
              "body": "Estimate $\\int_0^2 x^2\\,dx$ (exact value $\\tfrac{8}{3} \\approx 2.667$) with $n = 2$ rectangles, using left, right, and midpoint sums.",
              "solution": "<strong>Two subintervals,</strong> each of width $\\Delta x = 1$: $[0,1]$ and $[1,2]$, with $f(x) = x^2$.\n<strong>Left endpoints</strong> ($x = 0, 1$): $L = (0^2 + 1^2)\\cdot 1 = 1$ — an <em>under</em>estimate (the function is increasing, so the left edge is the lowest point of each strip).\n<strong>Right endpoints</strong> ($x = 1, 2$): $R = (1^2 + 2^2)\\cdot 1 = 5$ — an <em>over</em>estimate.\n<strong>Midpoints</strong> ($x = 0.5, 1.5$): $M = (0.5^2 + 1.5^2)\\cdot 1 = 0.25 + 2.25 = 2.5$ — much closer to the true $2.667$ than either endpoint rule.\n<strong>The aha.</strong> Left and right sums err in opposite directions and only as $O(1/n)$; the midpoint rule lets each rectangle's over- and under-shoot cancel within the strip, so its error shrinks as $O(1/n^2)$. All three converge to the same value as $n \\to \\infty$ — the definite integral <em>is</em> that limit — but where you sample inside each strip decides how fast you arrive."
            }
          ]
        },
        {
      ...
      "lessons": [
        ...
        },
        {
          ...
          "examples": [
            ...
            {
              "title": "An improper integral from a vertical asymptote",
              "body": "Evaluate $\\int_0^1 \\tfrac{1}{\\sqrt x}\\,dx$. The integrand blows up at $x = 0$ — does the area still converge?",
              "solution": "<strong>Improper at the lower limit.</strong> $\\tfrac{1}{\\sqrt x} \\to \\infty$ as $x \\to 0^+$, so this is an improper integral of the <em>second kind</em> (unbounded integrand at a finite endpoint, not an infinite limit). Handle it with a limit: $\\int_0^1 x^{-1/2}\\,dx = \\lim_{a \\to 0^+}\\int_a^1 x^{-1/2}\\,dx$.\n<strong>Compute.</strong> $\\int x^{-1/2}\\,dx = 2x^{1/2}$, so $\\int_a^1 = 2\\sqrt 1 - 2\\sqrt a = 2 - 2\\sqrt a$. As $a \\to 0^+$, $2\\sqrt a \\to 0$, giving $2$. The area is finite even though the function is unbounded.\n<strong>The mirror of the tail test.</strong> For $\\int_0^1 x^{-p}\\,dx$ the rule flips: it converges iff $p \\lt 1$ (here $p = \\tfrac12$), whereas the infinite tail $\\int_1^\\infty x^{-p}\\,dx$ converges iff $p \\gt 1$. A milder singularity ($p \\lt 1$) has integrable area; a steeper one ($p \\ge 1$, like $\\tfrac1x$) diverges.\n<strong>The aha.</strong> \"Improper\" means a limit is hiding somewhere — at infinity <em>or</em> at a point where the integrand explodes. Both are handled the same way: cut out the bad point, integrate, take the limit. Unbounded height does not preclude finite area, as long as the spike is thin enough."
            }
          ]
        },
        {
          ...
          "examples": [
            ...
            {
              "title": "Deriving Newton's law of cooling by separation",
              "body": "A hot object at initial temperature $T_0$ sits in a room at ambient temperature $T_a$. Newton's law of cooling says $\\dfrac{dT}{dt} = -k\\,(T - T_a)$ with $k>0$. Solve this separable equation to derive the formula $T(t) = T_a + (T_0 - T_a)e^{-kt}$.",
              "solution": "<strong>Separate — around the shifted variable.</strong> The rate depends on the <em>gap</em> $T - T_a$, not on $T$ itself, so collect that whole expression: $\\dfrac{dT}{T - T_a} = -k\\,dt$ (valid while $T \\neq T_a$).\n<strong>Integrate both sides.</strong> $\\ln|T - T_a| = -kt + C$. Exponentiate: $|T - T_a| = e^C e^{-kt}$, so $T - T_a = A\\,e^{-kt}$ where $A = \\pm e^C$ absorbs the sign.\n<strong>Apply the initial condition.</strong> At $t=0$: $T_0 - T_a = A e^0 = A$. Hence\n$$T(t) = T_a + (T_0 - T_a)\\,e^{-kt}.$$\n<strong>Read the formula.</strong> The temperature <em>gap</em> decays exponentially while the ambient level $T_a$ stays as an offset — the object relaxes toward the room, not toward zero. As $t\\to\\infty$, $T \\to T_a$: the equilibrium $T = T_a$ (where $\\frac{dT}{dt}=0$) is stable, and it is exactly the solution the separation step temporarily excluded.\n<strong>The aha.</strong> This is $y'=ky$ wearing a disguise: substitute $u = T - T_a$ and the equation becomes $u' = -ku$, pure exponential decay. Spotting the right <em>shifted</em> variable is the recurring trick — the same move that turns many \"almost linear\" rules into ones you already know how to solve."
            }
          ]
        }
      ]
      ...
      "lessons": [
        {
          ...
          "examples": [
            ...
            {
              "title": "The gradient points uphill: directional derivatives",
              "body": "For $f(x,y) = x^2 + y^2$, find the rate of change at $(1, 2)$ in the direction of $(3, 4)$, and the direction of steepest ascent.",
              "solution": "<strong>The gradient.</strong> The partials are $f_x = 2x$ and $f_y = 2y$, so $\\nabla f = (2x, 2y)$. At $(1, 2)$ that is $\\nabla f = (2, 4)$.\n<strong>Directional derivative = gradient dotted with a unit direction.</strong> Normalize: $(3,4)$ has length 5, so $\\hat{u} = (\\tfrac{3}{5}, \\tfrac{4}{5})$. Then\n$$D_{\\hat u} f = \\nabla f \\cdot \\hat u = 2\\cdot\\tfrac{3}{5} + 4\\cdot\\tfrac{4}{5} = \\tfrac{6 + 16}{5} = 4.4.$$\n<strong>Steepest ascent.</strong> Since $\\nabla f \\cdot \\hat u = |\\nabla f|\\cos\\theta$ is largest when $\\hat u$ points along $\\nabla f$, the gradient itself is the direction of fastest increase, and the maximum rate is $|\\nabla f| = \\sqrt{2^2 + 4^2} = \\sqrt{20} \\approx 4.47$. Perpendicular to $\\nabla f$ the rate is zero — you are walking along a level curve.\n<strong>The aha.</strong> The gradient packs every directional derivative into one vector: its <em>direction</em> is steepest ascent, its <em>length</em> is that steepest rate, and the rate along any unit $\\hat u$ is the projection $\\nabla f \\cdot \\hat u$. That is exactly why gradient descent steps along $-\\nabla f$ — straight downhill."
            }
          ]
        },
        {
          ...
          "examples": [
            ...
            {
              "title": "Moving along a level curve: the directional derivative is zero",
              "body": "For $f(x,y) = x^2 + y^2$, the gradient at $(1,1)$ is $\\nabla f = (2, 2)$. Find the directional derivative in the direction $\\hat u = \\tfrac{1}{\\sqrt 2}(1, -1)$, and say what the answer means.",
              "solution": "<strong>Dot the gradient with the unit direction.</strong> $D_{\\hat u} f = \\nabla f \\cdot \\hat u = 2\\cdot\\tfrac{1}{\\sqrt 2} + 2\\cdot\\tfrac{-1}{\\sqrt 2} = \\tfrac{2 - 2}{\\sqrt 2} = 0$.\n<strong>Zero means no change, to first order.</strong> The direction $(1, -1)$ is <em>perpendicular</em> to the gradient $(2, 2)$ (dot product 0), and along a direction perpendicular to $\\nabla f$ the function is momentarily flat — you are moving <b>along a level curve</b> (here the circle $x^2 + y^2 = 2$), where $f$ stays constant.\n<strong>The geometry of the gradient.</strong> $D_{\\hat u} f = |\\nabla f|\\cos\\theta$ is maximal along $\\nabla f$ ($\\theta = 0$, steepest ascent), minimal opposite it (steepest descent), and <em>zero</em> at right angles ($\\theta = 90°$). So the gradient is always perpendicular to the level curves it crosses.\n<strong>The aha.</strong> The gradient points straight uphill, and its perpendicular directions are exactly the ones that keep you at the same height. That orthogonality — gradient perpendicular to level set — is why contour lines and gradient arrows meet at right angles, and why constrained optimization (Lagrange) balances the two."
            }
          ]
        },
        {
    ...
    },
    {
      ...
      "lessons": [
        ...
        },
        {
          ...
          "minutes": 16,
          "content": "<h3>1. The hook: when does gradient descent actually work?</h3>\n<p>Gradient descent is the workhorse of machine learning, but on a general function it offers no promises — it can stall at a saddle or a shallow local dip. <strong>Convexity changes the contract.</strong> On a convex function, gradient descent is not just a heuristic that often works; it is a method with a <em>proof</em> that it converges to the global minimum, at a rate you can quantify in advance. This lesson connects the convexity of the last lesson to a concrete guarantee.</p>\n\n<h3>2. The algorithm, briefly</h3>\n<p><strong>Gradient descent</strong> repeatedly steps downhill, opposite the gradient:\n$$x_{k+1} = x_k - \\eta\\,\\nabla f(x_k),$$\nwhere $\\eta>0$ is the <strong>learning rate (step size)</strong>. The gradient points in the direction of steepest <em>increase</em>, so its negative is steepest decrease; $\\eta$ controls how far you move each step.</p>\n\n<h3>3. Convexity turns \"a local min\" into \"the answer\"</h3>\n<p>On a general landscape, the most gradient descent can promise is to reach a point where $\\nabla f = 0$ — which might be a local min, a saddle, or a plateau. On a <em>convex</em> function this is enough: a stationary point of a convex function is a global minimum (from the previous lesson). So if gradient descent converges at all on a convex problem, it converges to the global optimum. Convexity removes every trap that makes the algorithm's output untrustworthy.</p>\n\n<h3>4. The learning rate and smoothness</h3>\n<p>Convergence still hinges on choosing $\\eta$ well. The relevant property is <strong>smoothness</strong>: $f$ is $L$-smooth if its gradient doesn't change too fast (the gradient is $L$-Lipschitz, equivalently the Hessian's eigenvalues are bounded above by $L$). For an $L$-smooth convex function, a constant step size $\\eta \\le 1/L$ guarantees the loss decreases every step and converges. The failure modes are familiar:</p>\n<ul>\n<li>$\\eta$ <strong>too small</strong>: stable but painfully slow — many tiny steps.</li>\n<li>$\\eta$ <strong>too large</strong> (beyond $\\sim 2/L$): the steps overshoot the valley and the loss oscillates or diverges.</li>\n</ul>\n\n<h3>5. Strong convexity gives speed</h3>\n<p>Plain convexity guarantees you <em>arrive</em>; <strong>strong convexity</strong> guarantees you arrive <em>fast</em>. A function is $\\mu$-strongly convex if it curves up at least as much as a quadratic with curvature $\\mu$ (Hessian eigenvalues bounded <em>below</em> by $\\mu>0$) — it has a definite bowl shape, no flat directions. Strong convexity gives a unique minimum and, crucially, a fast convergence rate.</p>\n\n<p>See the geometry for yourself. Minimizing $f(x)=x^2$ (gradient $2x$) with step size $\\eta=0.1$, each update multiplies $x$ by $1-2\\eta=0.8$ — so the distance to the minimum shrinks by a constant factor every step. That constant-factor shrinkage <em>is</em> linear (geometric) convergence:</p>\n<div data-code=\"javascript\" data-expected=\"x: 10 -> 8 -> 6.4 -> 5.12 -> 4.096 -> 3.277\">// Gradient descent on f(x) = x^2 (gradient 2x), step size 0.1, starting at x = 10.\nlet x = 10, eta = 0.1;\nconst path = [x];\nfor (let i = 0; i &lt; 5; i++) {\n  x = x - eta * 2 * x;                 // x <- x - eta*f'(x)  =  0.8 * x\n  path.push(Math.round(x * 1000) / 1000);\n}\nconsole.log(\"x: \" + path.join(\" -> \"));   // each step multiplies x by 0.8 -> geometric convergence</div>\n<h3>6. Convergence rates</h3>\n<p>The rates make the distinction precise. For an $L$-smooth function with step $\\eta=1/L$:</p>\n<ul>\n<li><strong>Convex</strong>: the error shrinks like $f(x_k)-f^\\star = O(1/k)$ — a <em>sublinear</em> rate; halving the error takes twice as many steps.</li>\n<li><strong>Strongly convex</strong>: the error shrinks like $O\\big((1-\\mu/L)^k\\big)$ — a <em>linear</em> (geometric) rate; each step multiplies the error by a constant factor $<1$, so accuracy improves exponentially fast.</li>\n</ul>\n<p>The ratio $\\kappa = L/\\mu$ is the <strong>condition number</strong>: when $\\kappa$ is large (a long, narrow valley) convergence crawls and the iterates zig-zag; when $\\kappa\\approx1$ (a round bowl) it races straight to the bottom. This is exactly why <em>feature scaling</em> and <em>preconditioning</em> help — they shrink $\\kappa$.</p>\n<div class=\"callout sage\">\n<div class=\"c-tag\">Why this matters for ML</div>\n<p>It explains the whole toolkit: momentum and Adam fight ill-conditioning (large $\\kappa$); normalizing inputs rounds out the bowl; the learning rate must respect smoothness $L$ or training diverges. The convex theory is the clean model that the messier deep-learning practice imitates.</p>\n</div>\n\n<h3>7. But deep learning is non-convex — so why does GD work?</h3>\n<p>Neural-network losses are non-convex, so none of these global guarantees strictly apply. Yet gradient descent works remarkably well in practice, for reasons still being researched: in very high dimensions most critical points are <em>saddles</em> rather than bad local minima; massive <strong>over-parameterization</strong> smooths the landscape and creates wide, connected basins of good solutions; and the noise in <strong>stochastic</strong> gradient descent helps escape saddles and sharp minima. The convex picture remains the indispensable mental model — and locally, near a minimum, a smooth loss looks convex (its Hessian is PSD there).</p>\n\n<h3>8. Why this matters</h3>\n<p>Understanding convergence under convexity is what lets you reason about training instead of just turning knobs: it tells you why the learning rate has a ceiling, why a poorly scaled problem trains slowly, what momentum and adaptive methods are really fixing, and what \"convergence\" can and cannot be promised. It is the theoretical backbone beneath every optimizer you will ever use.</p>\n<details class=\"deep-dive\">\n<summary>Deeper dive: why gradient descent zigzags (the condition number)</summary>\n<p>On a nicely round bowl, gradient descent walks straight to the minimum. On a long, narrow valley it <em>zigzags</em> — ricocheting between the steep walls while creeping along the gentle floor. The culprit is the <b>condition number</b> $\\kappa = \\lambda_{\\max}/\\lambda_{\\min}$, the ratio of largest to smallest curvature (the Hessian's eigenvalues).</p>\n<p>A single learning rate has to serve every direction at once. To stay stable it must be small enough for the <em>steepest</em> direction ($\\lambda_{\\max}$), which leaves it far too small for the <em>flattest</em> ($\\lambda_{\\min}$) — so progress along the valley floor is throttled. The number of steps to converge grows with $\\kappa$: a well-conditioned problem ($\\kappa \\approx 1$) converges fast, while an ill-conditioned one ($\\kappa \\gg 1$) crawls.</p>\n<p>The \"aha\": the difficulty isn't the slope, it's the <em>mismatch</em> in curvature across directions. That is exactly what momentum, adaptive methods like Adam, and preconditioning address — they let each direction move at its own sensible pace instead of being shackled to one global step size.</p>\n</details>\n<h4>Interactive — the learning-rate regimes</h4>\n<div data-viz=\"calc-gd\"></div>\n<details class=\"deep-dive\">\n<summary>Deeper dive: momentum smooths the zigzag</summary>\n<p>The first dive showed <em>why</em> gradient descent zigzags: in an ill-conditioned valley (large condition number $\\kappa$) the steepest direction and the direction to the minimum disagree, so steps oscillate across the narrow axis while crawling along the long one. <b>Momentum</b> is the fix.</p>\n<p>Momentum keeps a velocity $v \\leftarrow \\beta v + \\nabla f$ and steps with $v$ instead of the raw gradient. Across the oscillating axis, successive gradients point in <em>opposite</em> directions and largely cancel in the running average; along the shallow axis they point the <em>same</em> way and accumulate. The result is damped oscillation and accelerated progress toward the minimum — effectively a larger usable step without diverging. For a quadratic, momentum improves the convergence rate's dependence on $\\kappa$ to roughly $\\sqrt{\\kappa}$.</p>\n<p>The \"aha\": momentum is a low-pass filter on the gradient — it averages out the high-frequency zigzag and amplifies the consistent low-frequency drift. That is why nearly every modern optimizer (Adam included) carries a momentum term: it directly attacks the conditioning problem that makes plain gradient descent slow.</p>\n</details>\n<details class=\"deep-dive\">\n<summary>Deeper dive: the convergence rate and why preconditioning helps</summary>\n<p>The zigzag has a precise cost: on a convex quadratic, gradient descent's error shrinks by a <em>fixed factor each step</em>, and that factor is set entirely by the <b>condition number</b> $\\kappa = \\lambda_{\\max}/\\lambda_{\\min}$.</p>\n<p><b>The rate.</b> With a well-tuned step size, the distance to the minimum contracts like $\\left(\\dfrac{\\kappa-1}{\\kappa+1}\\right)$ per step. When $\\kappa$ is small (well-conditioned, $\\approx 1$) the factor is near 0 — convergence in a few steps. When $\\kappa$ is large (a long thin valley, say $\\kappa=100$) the factor is $\\approx 0.98$ — it crawls, shaving off under 2% per step. The condition number does not just <em>cause</em> the zigzag; it <em>quantifies</em> how slow it is.</p>\n<p><b>Two ways to speed up.</b> <em>Momentum</em> improves the dependence from $\\kappa$ to $\\sqrt{\\kappa}$ (for $\\kappa=100$, the factor drops from $0.98$ to about $0.82$ — dramatically faster). <em>Preconditioning</em> attacks $\\kappa$ directly: rescale the problem so its curvature is more uniform. Normalizing or whitening inputs, batch norm, and Adam's per-coordinate scaling are all cheap preconditioners that shrink the <em>effective</em> condition number — which is the real reason they accelerate training.</p>\n<p>The \"aha\": convergence speed is governed by a single number, $\\kappa$. You either lower it (precondition: normalize, Adam) or weaken its grip (momentum: $\\kappa \\to \\sqrt{\\kappa}$). \"Make the bowl rounder, or build up speed across it\" — both target the same enemy, the condition number.</p>\n</details>\n<details class=\"deep-dive\">\n<summary>Deeper dive: Robbins-Monro — why SGD with a decaying step size actually converges</summary>\n<p>Stochastic gradient descent updates $x_{t+1} = x_t - a_t\\,g_t$ where $g_t$ is only a <em>noisy</em> estimate of the true gradient (one minibatch's worth). Why should following noisy directions land you at the true optimum? The answer predates deep learning by decades: the <b>Robbins-Monro</b> stochastic-approximation theorem (1951), which pins convergence on two conditions for the step-size schedule $a_t$.</p>\n<p>First, $\\sum_t a_t = \\infty$: the steps must sum to infinity, so the iterate can travel arbitrarily far and reach the optimum from any starting point. Second, $\\sum_t a_t^2 < \\infty$: the squared steps must be finite, so the injected gradient noise is summable and eventually averages out instead of accumulating. The classic schedule $a_t = 1/t$ threads the needle exactly — the harmonic series diverges while $\\sum 1/t^2$ converges.</p>\n<p>That is precisely why <a href=\"#/lesson/deep-learning/dl-learning-rate-schedules-and-tuning\" data-route>learning-rate schedules</a> <em>decay</em>: a constant rate violates the second condition, so SGD never settles — it rattles forever inside a noise ball whose radius is proportional to the step size. The first condition demands you keep moving; the second demands you slow down enough to converge. (In practice we often keep a small constant rate or warm-restart, since a tight noise ball is good enough and sometimes even helps generalization — but Robbins-Monro is the theory underneath every schedule.)</p>\n</details>\n<details class=\"deep-dive\">\n<summary>Deeper dive: line search and the Wolfe conditions — how far to step</summary>\n<p>Every descent method first picks a direction $p$ (the negative gradient, a Newton step, a quasi-Newton step) and then must choose how far to move along it: $x \\leftarrow x + \\alpha p$. Too small a step $\\alpha$ wastes iterations; too large overshoots or diverges. Choosing $\\alpha$ is the job of <b>line search</b>. Exact line search minimizes $f(x+\\alpha p)$ over $\\alpha$ — usually far too costly — so in practice one finds a step that is merely <em>good enough</em>.</p>\n<p>\"Good enough\" is pinned down by the <b>Wolfe conditions</b>. Sufficient decrease (the Armijo rule) demands $f(x+\\alpha p)\\le f(x)+c_1\\,\\alpha\\,\\nabla f^\\top p$: the drop must be proportional to both the step and the initial slope, ruling out steps that barely improve. The curvature condition $\\nabla f(x+\\alpha p)^\\top p \\ge c_2\\,\\nabla f^\\top p$ rules out stopping while still descending steeply, so the slope has flattened enough. With $0 < c_1 < c_2 < 1$ they bracket a genuinely useful step.</p>\n<p>The cheap workhorse is <b>backtracking</b>: start at $\\alpha=1$ and halve until Armijo holds — simple and robust. The curvature half matters most for quasi-Newton methods, where it keeps the Hessian approximation positive definite. And this is the rigorous form of a knob you already know: a deep net's fixed or scheduled <a href=\"#/lesson/deep-learning/dl-learning-rate-schedules-and-tuning\" data-route>learning rate</a> is a cheap stand-in for line search, which is too expensive to run on every minibatch.</p>\n</details>\n<div data-viz=\"c-sgd-noise-ball\"></div>\n",
          "mcq": [
            {
          ...
          "examples": [
            ...
            {
              "title": "The learning-rate window: too big and it diverges",
              "body": "Gradient descent on $f(x) = \\tfrac{1}{2}a x^2$ (with $a = 2$) updates $x \\leftarrow x - \\eta f'(x)$. For which learning rates $\\eta$ does it converge, and which is fastest?",
              "solution": "<strong>One step is a multiplication.</strong> Since $f'(x) = a x$, the update is $x \\leftarrow x - \\eta a x = (1 - \\eta a)\\,x$ — each step multiplies $x$ by the fixed factor $(1 - \\eta a)$.\n<strong>Convergence needs $|1 - \\eta a| \\lt 1$.</strong> The iterates shrink to 0 only if that factor has magnitude below 1: $-1 \\lt 1 - \\eta a \\lt 1$, i.e. $0 \\lt \\eta \\lt \\tfrac{2}{a}$. With $a = 2$ that is $0 \\lt \\eta \\lt 1$. At $\\eta \\ge \\tfrac{2}{a}$ the factor is $\\le -1$ and the iterates <em>grow</em> — gradient descent diverges, overshooting more each step.\n<strong>The fastest rate.</strong> The factor is smallest (zero) at $\\eta = \\tfrac{1}{a} = 0.5$, landing on the minimum in a <em>single</em> step. Below it you converge monotonically; between $\\tfrac{1}{a}$ and $\\tfrac{2}{a}$ you converge while flipping sign each step.\n<strong>The aha.</strong> The safe rate is set by the curvature $a$ (the second derivative): $\\eta \\lt 2/a$. In many dimensions $a$ becomes the largest Hessian eigenvalue, so the step that keeps the steepest direction stable bounds the whole rate — and a wide spread of curvatures (a large condition number) is what forces a small, slow $\\eta$."
            }
          ]
        },
        {
          ...
          "examples": [
            {
              ...
              "solution": "Lagrangian: $\\mathcal{L}=\\ell w-\\lambda(\\ell+2w-40)$. Stationarity: $\\partial_\\ell\\mathcal{L}=w-\\lambda=0$ and $\\partial_w\\mathcal{L}=\\ell-2\\lambda=0$, so $w=\\lambda$ and $\\ell=2\\lambda$. Constraint: $\\ell+2w=2\\lambda+2\\lambda=4\\lambda=40\\Rightarrow\\lambda=10$. Thus $w=10$, $\\ell=20$, and $A=20\\times10=200\\,\\text{m}^2$.\n\nInterpretation: $\\lambda=10$ is the shadow price of fence — one extra meter of fence would add about $10\\,\\text{m}^2$ of area at the optimum. (Indeed re-solving with 41 m gives $\\lambda=\\tfrac{41}{4}=10.25$ and $A=2\\lambda^2=210.125$ — an increase of $10.125$, matching $\\lambda=10$ to first order.) The structural result — the optimal length equals twice the width — falls straight out of the multiplier conditions."
            },
            {
            ...
            {
              "title": "The multiplier λ is a shadow price",
              "body": "Maximize $f = xy$ subject to $x + y = S$. Beyond solving it, what does the Lagrange multiplier $\\lambda$ actually <em>mean</em>?",
        ...
        },
        {
          ...
          "mcq": [
            ...
            {
              "q": "Frank-Wolfe stays feasible without ever projecting because:",
              "choices": [
                "It shrinks the step size to zero",
                "It only moves along coordinate axes",
                "Each iterate is a convex combination of the current point and a point of the set returned by a linear minimization oracle",
                "It adds a penalty term for leaving the set"
              ],
              "answer": 2,
