...
- [x] **S3 (iter 1218; MCQ-parity backlog folds into the P1 rewrites, enforced by gate allowlist) · One-sweep data-shape hygiene + gate enforcement**: (a) add missing `id` fields to
...
- [x] (iter 1218 — app.js lookups now quote-agnostic at all 3 sites) Four single-quoted `data-viz='…'` embeds (ts-acf, ts-forecast-cone, gt-replicator,
...
- [x] (iter 1218 — Shapley widget+try-it → gt-cooperative-games §1; Vickrey try-it → gt-auctions §1) gt-shapley widget stranded in gt-repeated-cooperation; move to gt-cooperative-games (and
  the Vickrey try-it from gt-foundations to the auctions lesson).
- [x] (iter 1218 — window.typeset(el) exposed from app.js; dl-optimizers + calc-gd2d call it on note update, dl-rectified-flow's existing dead guard came alive) Raw `$…# HOLISTIC.md — site-wide findings & radical-change backlog
...
- [x] (iter 1218 — Shapley widget+try-it → gt-cooperative-games §1; Vickrey try-it → gt-auctions §1) gt-shapley widget stranded in gt-repeated-cooperation; move to gt-cooperative-games (and
  the Vickrey try-it from gt-foundations to the auctions lesson).
 LaTeX shows in viz notes after first interaction (dl-optimizers, calc-gd2d,
  dl-rectified-flow) → re-typeset notes on update or strip math from notes.
- [x] (iter 1218 — calc-gd2d pointerdown; llm-attention tap-to-inspect/tap-to-clear) Touch-dead interactions: calc-gd2d (mousedown-only; its note says "click anywhere" but touch
